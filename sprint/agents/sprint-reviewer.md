...
- **Oh My Pi:** dispatch all workers in **one `task` batch call**, each item using the flat read-only agent `reviewer`. Put `runtime: omp`, the plan path, and the worker's scope in every task text. Workers run at the `reviewer` definition's bound model; pass no model anywhere. Never use an `eval` cell or the `agent()` bridge.
...
1. Dispatch one write-capable fixer with `runtime`, the plan path, and the deduplicated supported findings and evidence in its prompt (plus the exact `review-model` on Claude). The fixer must work only in `cwd`, apply source fixes and necessary tests, preserve the plan, run focused checks where practical, and leave changes uncommitted. It must not weaken requirements or tests, commit, ask the user, or make unrelated cleanup.
2. Dispatch focused re-reviewers only for the backend axes and risk scopes implicated by the findings or fixer changes. Include `runtime`, the plan path, the prior finding, its evidence, the fixer's summary, and the current changed-file list in every prompt (plus the exact `review-model` on Claude). Require each worker to inspect the actual post-fix diff and report `resolved`, a still-supported finding with current evidence, or a concrete regression introduced by the fix.
...
- **Oh My Pi:** dispatch the fixer through `task` with the flat write-capable agent `task`; dispatch independent focused re-reviewers in one `task` batch with flat agent `reviewer`. Pass no model anywhere; workers run at their definitions' bound models.
...
- No unbounded repair loop, model selection on OMP, model inheritance or downgrade on Claude, or user question.
