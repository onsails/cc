...
| executor SDD: codex | *(engine=codex)* `fd -t d subagent-driven-development ~/.codex/skills ~/.claude/plugins/marketplaces/openai-codex 2>/dev/null` | found → append `Use subagent-driven-development.` to the codex prompt (§4a); absent → hand codex the whole plan |
| executor SDD: mimo | *(engine=mimo)* `find "${XDG_DATA_HOME:-$HOME/.local/share}/mimocode" -path '*/skills/subagent/SKILL.md' 2>/dev/null` (mimo ships `compose:subagent`, and also loads Claude skills) | found → append the SDD line to the mimo prompt (§4b); absent (e.g. mimo run with `--pure`) → plain plan |
| executor SDD: native | *(engine=native)* `superpowers:subagent-driven-development` in the skills list? | yes → pass `sdd: available` to `stage-executor`; it uses SDD **iff it actually holds the `Agent` tool** (nesting may withhold it — §4c), else implements directly |
