...
- `LAUNCHER` — absolute path to `scripts/mimo-run.mjs` in the mimo-code plugin.

## What you do
1. Run EXACTLY ONE foreground Bash call (set a generous `timeout`, up to the max):
...
2. Read the streamed NDJSON. Determine whether mimo finished (a `step_finish`
   with `reason: "stop"`), errored, or was cut off (timeout / non-zero exit).
3. Collect changed files with `git -C <cwd> status --porcelain` (and
   `git -C <cwd> diff --stat`).
